...
// Fallback high-quality placeholder images for events
...
export default function EventSubpage(props: { params: Promise<{ slug: string }> }) {
  ...
  useEffect(() => {
    async function loadPage() {
      ...
      try {
        ...
        if (!res.ok) {
          if (res.status === 404) {
            throw new Error('This event page does not exist or has not been announced.');
          }
          throw new Error('Failed to load event page content.');
        }
  ...
  if (error || !pageData) {
    return (
      <div className={styles.errorState}>
        ...
        <h2 className={styles.errorTitle}>Event Registry Offline</h2>
        <p>{error || 'Event information missing.'}</p>
      </div>
    );
  }
