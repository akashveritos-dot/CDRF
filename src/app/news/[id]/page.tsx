...
const getTagColor = (tag: string) => {
  switch ((tag || '').toLowerCase()) {
    case 'breaking': return 'var(--red-primary)';
    case 'environment': return 'var(--teal-primary)';
    case 'health crisis': return 'var(--orange-primary)';
    case 'climate': return 'var(--purple-primary)';
    case 'disasters': return 'var(--red-primary)';
    default: return 'var(--teal-primary)';
  }
};
...
export default async function NewsDetailsPage(props: { params: Promise<{ id: string }> }) {
  ...
  try {
    ...
    // Fetch up to 4 other stories for the sidebar
    const recentRows = await query<any[]>('SELECT * FROM news WHERE id != ? ORDER BY published_date DESC, id DESC LIMIT 4', [id]);
    if (Array.isArray(recentRows)) {
      recentStories = recentRows.map(r => ({
        ...r,
        date: new Date(r.published_date).toLocaleDateString('en-US', {
          year: 'numeric',
          month: 'short',
          day: 'numeric'
        })
        ...
        {recentStories.length > 0 && (
          <aside className={styles.sidebar}>
            <div className={styles.sidebarSection}>
              <h3 className={styles.sidebarSectionTitle}>Latest Updates</h3>
              <div className={styles.recentList}>
                {recentStories.map((item) => (
                  <Link key={item.id} href={`/news/${item.id}`} className={styles.recentItem}>
                    <div className={styles.recentItemMeta}>
                      <span className={styles.recentItemTag} style={{ color: getTagColor(item.tag) }}>{item.tag}</span>
                      <span>•</span>
                      <span>{item.date}</span>
                    </div>
                    <h4 className={styles.recentItemTitle}>{item.headline}</h4>
                    <p className={styles.recentItemExcerpt}>{item.excerpt}</p>
                  </Link>
                ))}
              </div>
            </div>
