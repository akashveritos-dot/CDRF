...
function generateSQL() {
  const sqlLines = [];
  sqlLines.push('INSERT INTO `email_templates` (`template_key`, `name`, `subject`, `body`) VALUES');
  ...
  const values = templates.map(t => {
    ...
    return `('${key}', '${name}', '${subject}', '${body}')`;
  });
  
  sqlLines.push(values.join(',\n') + ';');
  const fullSql = sqlLines.join('\n');
  
  const outputPath = require('path').join(__dirname, 'templates.sql');
  require('fs').writeFileSync(outputPath, fullSql, 'utf8');
  console.log('✓ SQL output generated at ' + outputPath);
}
